...
@php
    ...
    $positiveRate = $totalReviews > 0 ? round($reviews->where('rating', '>=', 4)->count() / $totalReviews * 100) : 0;
@endphp
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
    <div class="bg-white rounded-2xl p-6 border border-border shadow-sm flex items-start gap-4">
        ...
        <div>
            ...
            <div class="text-[11px] font-medium text-muted">{{ number_format($totalReviews) }} total reviews</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 border border-border shadow-sm flex items-start gap-4">
        ...
        <div>
            <div class="text-[28px] font-serif font-bold text-dark leading-none mb-1">{{ $positiveRate }}%</div>
...
<div class="bg-white rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col">
    ...
    <div class="divide-y divide-border" id="reviews-container">
        ...
        <div class="review-row hover:bg-cream/30 transition-colors p-6 flex gap-4 md:gap-6 items-start" id="review-row-{{ $r->id }}">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="font-bold text-[14.5px] text-dark">{{ $r->user->name }}</span>
                        ...
                        <span class="font-bold text-[13.5px] text-brand">{{ $r->muaProfile->user->name }}</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <span class="text-[12px] text-muted">{{ $r->created_at->format('d M Y') }}</span>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-1 mb-3">
                    @for($i=1;$i<=5;$i++)
                        ...
                        @endif
                    @endfor
                    <span class="text-[12px] font-bold text-dark ml-1">{{ $r->rating }}.0</span>
                </div>
                
                @if($r->comment)
                <p class="text-[14px] text-muted leading-relaxed mb-4">"{{ $r->comment }}"</p>
                @endif
                
                <div class="flex flex-wrap gap-2">
                    <form action="{{ route('admin.reviews.destroy', $r->id) }}" method="POST" onsubmit="return confirm('Delete this review?')">
                        @csrf @method('DELETE')
                        <button type="submit" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white font-bold text-[12px] transition-colors">
