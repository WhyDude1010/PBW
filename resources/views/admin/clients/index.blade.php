                ...
                <tbody class="divide-y divide-border">
                    ...
                    @foreach($clients as $c)
                        <tr class="hover:bg-cream/30 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-dark/5 text-dark uppercase tracking-wider">{{ $c[3] }}</span>
                            </td>
                            <td class="px-6 py-4 text-[14px] font-bold text-brand">{{ $c[4] }}</td>
                            <td class="px-6 py-4 text-[13px] text-muted">{{ $c[5] }}</td>
                            <td class="px-6 py-4 text-right">
                                <button @click="viewClient('{{ $c[0] }}', '{{ $c[1] }}', '{{ $c[2] }}', '{{ $c[3] }}', '{{ $c[4] }}', '{{ $c[5] }}')" class="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-cream text-muted hover:bg-dark hover:text-white font-bold text-[12px] transition-colors">
